...
+ (void)listMethodsIn:(Class)aClass isInstance:(BOOL)isInstance {
    ...
    for (unsigned int i = 0; i < methodCount; i++) {
        if (isInstance) {
            NSLog(@"%s Instance : %s, %p", class_getName(aClass), sel_getName(method_getName(methodList[i])), method_getImplementation(methodList[i]));
        }
        else {
            NSLog(@"%s Class : %s, %p", class_getName(aClass), sel_getName(method_getName(methodList[i])), method_getImplementation(methodList[i]));
        }
    }
    free(methodList);
}
